...
#ifdef __ANDROID__
static PFN_vkCreateAndroidSurfaceKHR android_vkCreateAndroidSurfaceKHR;
static PFN_vkGetPhysicalDeviceSurfaceSupportKHR android_vkGetPhysicalDeviceSurfaceSupportKHR;
static PFN_vkGetPhysicalDeviceSurfaceCapabilitiesKHR android_vkGetPhysicalDeviceSurfaceCapabilitiesKHR;
static PFN_vkGetPhysicalDeviceSurfaceFormatsKHR android_vkGetPhysicalDeviceSurfaceFormatsKHR;
static PFN_vkGetPhysicalDeviceSurfacePresentModesKHR android_vkGetPhysicalDeviceSurfacePresentModesKHR;
static PFN_vkDestroySurfaceKHR android_vkDestroySurfaceKHR;
static PFN_vkCreateSwapchainKHR android_vkCreateSwapchainKHR;
static PFN_vkDestroySwapchainKHR android_vkDestroySwapchainKHR;
static PFN_vkGetSwapchainImagesKHR android_vkGetSwapchainImagesKHR;
static PFN_vkAcquireNextImageKHR android_vkAcquireNextImageKHR;
static PFN_vkQueuePresentKHR android_vkQueuePresentKHR;

static bool pgraph_vk_android_load_present_procs(PGRAPHVkState *r)
{
    if (android_vkCreateAndroidSurfaceKHR) {
        return true;
    }

    android_vkCreateAndroidSurfaceKHR =
        (PFN_vkCreateAndroidSurfaceKHR)vkGetInstanceProcAddr(
            r->instance, "vkCreateAndroidSurfaceKHR");
    android_vkGetPhysicalDeviceSurfaceSupportKHR =
        (PFN_vkGetPhysicalDeviceSurfaceSupportKHR)vkGetInstanceProcAddr(
            r->instance, "vkGetPhysicalDeviceSurfaceSupportKHR");
    android_vkGetPhysicalDeviceSurfaceCapabilitiesKHR =
        (PFN_vkGetPhysicalDeviceSurfaceCapabilitiesKHR)vkGetInstanceProcAddr(
            r->instance, "vkGetPhysicalDeviceSurfaceCapabilitiesKHR");
    android_vkGetPhysicalDeviceSurfaceFormatsKHR =
        (PFN_vkGetPhysicalDeviceSurfaceFormatsKHR)vkGetInstanceProcAddr(
            r->instance, "vkGetPhysicalDeviceSurfaceFormatsKHR");
    android_vkGetPhysicalDeviceSurfacePresentModesKHR =
        (PFN_vkGetPhysicalDeviceSurfacePresentModesKHR)vkGetInstanceProcAddr(
            r->instance, "vkGetPhysicalDeviceSurfacePresentModesKHR");
    android_vkDestroySurfaceKHR =
        (PFN_vkDestroySurfaceKHR)vkGetInstanceProcAddr(
            r->instance, "vkDestroySurfaceKHR");
    android_vkCreateSwapchainKHR =
        (PFN_vkCreateSwapchainKHR)vkGetDeviceProcAddr(
            r->device, "vkCreateSwapchainKHR");
    android_vkDestroySwapchainKHR =
        (PFN_vkDestroySwapchainKHR)vkGetDeviceProcAddr(
            r->device, "vkDestroySwapchainKHR");
    android_vkGetSwapchainImagesKHR =
        (PFN_vkGetSwapchainImagesKHR)vkGetDeviceProcAddr(
            r->device, "vkGetSwapchainImagesKHR");
    android_vkAcquireNextImageKHR =
        (PFN_vkAcquireNextImageKHR)vkGetDeviceProcAddr(
            r->device, "vkAcquireNextImageKHR");
    android_vkQueuePresentKHR =
        (PFN_vkQueuePresentKHR)vkGetDeviceProcAddr(
            r->device, "vkQueuePresentKHR");

    bool ok = android_vkCreateAndroidSurfaceKHR &&
              android_vkGetPhysicalDeviceSurfaceSupportKHR &&
              android_vkGetPhysicalDeviceSurfaceCapabilitiesKHR &&
              android_vkGetPhysicalDeviceSurfaceFormatsKHR &&
              android_vkGetPhysicalDeviceSurfacePresentModesKHR &&
    ...
    if (!ok) {
        xemu_android_log_warn(
            "Android Vulkan present procs missing: createSurface=%p support=%p caps=%p formats=%p modes=%p destroySurface=%p createSwap=%p destroySwap=%p images=%p acquire=%p present=%p",
            android_vkCreateAndroidSurfaceKHR,
            android_vkGetPhysicalDeviceSurfaceSupportKHR,
            android_vkGetPhysicalDeviceSurfaceCapabilitiesKHR,
            android_vkGetPhysicalDeviceSurfaceFormatsKHR,
...
#if HAVE_EXTERNAL_MEMORY
    external_memory_image_create_info = (VkExternalMemoryImageCreateInfo){
...
#if HAVE_EXTERNAL_MEMORY
    export_memory_alloc_info = (VkExportMemoryAllocateInfo){
...
#ifdef __ANDROID__
static void pgraph_vk_android_destroy_swapchain(PGRAPHState *pg)
{
    PGRAPHVkState *r = pg->vk_renderer_state;
    PGRAPHVkDisplayState *disp = &r->display;

    if (disp->render_finished != VK_NULL_HANDLE) {
        vkDestroySemaphore(r->device, disp->render_finished, NULL);
        disp->render_finished = VK_NULL_HANDLE;
    }

    if (disp->image_acquired_fence != VK_NULL_HANDLE) {
        vkDestroyFence(r->device, disp->image_acquired_fence, NULL);
        disp->image_acquired_fence = VK_NULL_HANDLE;
    }

    for (uint32_t i = 0; i < disp->swapchain_image_count; i++) {
        if (disp->swapchain_image_views[i] != VK_NULL_HANDLE) {
            vkDestroyImageView(r->device, disp->swapchain_image_views[i], NULL);
            disp->swapchain_image_views[i] = VK_NULL_HANDLE;
        }
        disp->swapchain_images[i] = VK_NULL_HANDLE;
        disp->swapchain_image_layouts[i] = VK_IMAGE_LAYOUT_UNDEFINED;
    }
    disp->swapchain_image_count = 0;

    if (disp->swapchain != VK_NULL_HANDLE) {
        if (pgraph_vk_android_load_present_procs(r)) {
            android_vkDestroySwapchainKHR(r->device, disp->swapchain, NULL);
        }
        disp->swapchain = VK_NULL_HANDLE;
    }

    if (disp->android_surface != VK_NULL_HANDLE) {
        if (pgraph_vk_android_load_present_procs(r)) {
            android_vkDestroySurfaceKHR(r->instance, disp->android_surface, NULL);
        }
        disp->android_surface = VK_NULL_HANDLE;
    }

    disp->swapchain_ready = false;
}

static bool pgraph_vk_android_create_swapchain(PGRAPHState *pg)
{
    PGRAPHVkState *r = pg->vk_renderer_state;
    PGRAPHVkDisplayState *disp = &r->display;
    ANativeWindow *window = xemu_android_acquire_native_window();
    ...
    if (window == NULL) {
        xemu_android_log_warn("Android Vulkan swapchain: native window not ready");
        return false;
    }

    pgraph_vk_android_destroy_swapchain(pg);

    VkAndroidSurfaceCreateInfoKHR surface_info = {
        .sType = VK_STRUCTURE_TYPE_ANDROID_SURFACE_CREATE_INFO_KHR,
        .window = window,
    };
    VkResult result = android_vkCreateAndroidSurfaceKHR(
        r->instance, &surface_info, NULL, &disp->android_surface);
    xemu_android_release_native_window(window);
    if (result != VK_SUCCESS) {
        xemu_android_log_warn("Android Vulkan swapchain: vkCreateAndroidSurfaceKHR failed: %d", result);
        return false;
    }
    ...
    result = android_vkGetPhysicalDeviceSurfaceSupportKHR(
        r->physical_device, queue_indices.queue_family, disp->android_surface,
        &surface_supported);
    if (result != VK_SUCCESS || !surface_supported) {
        xemu_android_log_warn("Android Vulkan swapchain: queue family cannot present: result=%d supported=%d",
                              result, surface_supported);
        pgraph_vk_android_destroy_swapchain(pg);
        return false;
    }

    VkSurfaceCapabilitiesKHR caps;
    result = android_vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        r->physical_device, disp->android_surface, &caps);
    if (result != VK_SUCCESS) {
        xemu_android_log_warn("Android Vulkan swapchain: surface capabilities failed: %d", result);
        pgraph_vk_android_destroy_swapchain(pg);
        return false;
    }
    ...
    android_vkGetPhysicalDeviceSurfaceFormatsKHR(
        r->physical_device, disp->android_surface, &format_count, NULL);
    if (format_count == 0) {
        xemu_android_log_warn("Android Vulkan swapchain: no surface formats");
        pgraph_vk_android_destroy_swapchain(pg);
        return false;
    }

    g_autofree VkSurfaceFormatKHR *formats =
        g_new0(VkSurfaceFormatKHR, format_count);
    android_vkGetPhysicalDeviceSurfaceFormatsKHR(
        r->physical_device, disp->android_surface, &format_count, formats);
    ...
    android_vkGetPhysicalDeviceSurfacePresentModesKHR(
        r->physical_device, disp->android_surface, &present_mode_count, NULL);
    g_autofree VkPresentModeKHR *present_modes =
        g_new0(VkPresentModeKHR, present_mode_count);
    if (present_mode_count > 0) {
        android_vkGetPhysicalDeviceSurfacePresentModesKHR(
            r->physical_device, disp->android_surface, &present_mode_count,
            present_modes);
    }
    ...
    if (!(caps.supportedUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT)) {
        xemu_android_log_warn("Android Vulkan swapchain: TRANSFER_DST unsupported");
        pgraph_vk_android_destroy_swapchain(pg);
        return false;
    }
    ...
    if (extent.width == UINT32_MAX) {
        ANativeWindow *extent_window = xemu_android_acquire_native_window();
        int width = extent_window ? ANativeWindow_getWidth(extent_window) : 640;
        int height = extent_window ? ANativeWindow_getHeight(extent_window) : 480;
        xemu_android_release_native_window(extent_window);
    ...
    uint32_t image_count = MAX(2, caps.minImageCount + 1);
    if (caps.maxImageCount > 0) {
        image_count = MIN(image_count, caps.maxImageCount);
    }
    image_count = MIN(image_count, XEMU_ANDROID_SWAPCHAIN_MAX_IMAGES);

    VkSwapchainCreateInfoKHR swapchain_info = {
        .sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
        .surface = disp->android_surface,
    ...
    result = android_vkCreateSwapchainKHR(r->device, &swapchain_info, NULL,
                                          &disp->swapchain);
    if (result != VK_SUCCESS) {
        xemu_android_log_warn("Android Vulkan swapchain: vkCreateSwapchainKHR failed: %d", result);
        pgraph_vk_android_destroy_swapchain(pg);
        return false;
    }

    android_vkGetSwapchainImagesKHR(r->device, disp->swapchain,
                                    &disp->swapchain_image_count, NULL);
    disp->swapchain_image_count = MIN(disp->swapchain_image_count,
                                      XEMU_ANDROID_SWAPCHAIN_MAX_IMAGES);
    android_vkGetSwapchainImagesKHR(r->device, disp->swapchain,
                                    &disp->swapchain_image_count,
                                    disp->swapchain_images);

    for (uint32_t i = 0; i < disp->swapchain_image_count; i++) {
        VkImageViewCreateInfo image_view_info = {
            .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
            .image = disp->swapchain_images[i],
            .viewType = VK_IMAGE_VIEW_TYPE_2D,
        ...
        VK_CHECK(vkCreateImageView(r->device, &image_view_info, NULL,
                                   &disp->swapchain_image_views[i]));
        disp->swapchain_image_layouts[i] = VK_IMAGE_LAYOUT_UNDEFINED;
    }
    ...
    VK_CHECK(vkCreateSemaphore(r->device, &semaphore_info, NULL,
                               &disp->render_finished));

    VkFenceCreateInfo fence_info = {
        .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO,
    };
    VK_CHECK(vkCreateFence(r->device, &fence_info, NULL,
                           &disp->image_acquired_fence));

    disp->swapchain_format = chosen_format.format;
    disp->swapchain_extent = extent;
    disp->swapchain_ready = true;
    disp->swapchain_window_generation =
        xemu_android_get_native_window_generation();
    xemu_android_log_info("Android Vulkan swapchain ready: %ux%u images=%u format=%d presentMode=%d",
                          extent.width, extent.height,
                          disp->swapchain_image_count, chosen_format.format,
                          present_mode);
    return true;
}

static bool pgraph_vk_android_begin_present_commands(PGRAPHState *pg,
                                                     VkCommandBuffer *cmd)
{
    PGRAPHVkState *r = pg->vk_renderer_state;
    ...
    if (result != VK_SUCCESS) {
        ...
        xemu_android_log_warn("Android Vulkan present: begin command buffer failed: %d", result);
        return false;
    }

    *cmd = r->aux_command_buffer;
    return true;
}

static bool pgraph_vk_android_end_present_commands(PGRAPHState *pg,
                                                   VkCommandBuffer cmd)
{
    PGRAPHVkState *r = pg->vk_renderer_state;
    PGRAPHVkDisplayState *disp = &r->display;
    ...
    if (result != VK_SUCCESS) {
        r->in_aux_command_buffer = false;
        xemu_android_log_warn("Android Vulkan present: end command buffer failed: %d", result);
        return false;
    }
    ...
    VkSubmitInfo submit_info = {
        ...
        .pSignalSemaphores = &disp->render_finished,
    };
    ...
    r->in_aux_command_buffer = false;
    if (result != VK_SUCCESS) {
        xemu_android_log_warn("Android Vulkan present: queue submit failed: %d", result);
        return false;
    }
...
static void pgraph_vk_android_present_placeholder(PGRAPHState *pg)
{
    PGRAPHVkState *r = pg->vk_renderer_state;
    PGRAPHVkDisplayState *disp = &r->display;

    if (disp->swapchain_ready &&
        disp->swapchain_window_generation !=
            xemu_android_get_native_window_generation()) {
        pgraph_vk_android_destroy_swapchain(pg);
    }
    ...
    if (!disp->swapchain_ready &&
        !pgraph_vk_android_create_swapchain(pg)) {
        return;
    }

    uint32_t image_index = 0;
    VkResult result = android_vkAcquireNextImageKHR(
        r->device, disp->swapchain, UINT64_MAX, VK_NULL_HANDLE,
        disp->image_acquired_fence, &image_index);
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        pgraph_vk_android_create_swapchain(pg);
        return;
    }
    if (result != VK_SUCCESS && result != VK_SUBOPTIMAL_KHR) {
        xemu_android_log_warn("Android Vulkan placeholder: acquire failed: %d", result);
        return;
    }

    vkWaitForFences(r->device, 1, &disp->image_acquired_fence, VK_TRUE,
                    UINT64_MAX);
    vkResetFences(r->device, 1, &disp->image_acquired_fence);

    VkCommandBuffer cmd;
    if (!pgraph_vk_android_begin_present_commands(pg, &cmd)) {
        return;
    }

    VkImageLayout old_swap_layout = disp->swapchain_image_layouts[image_index];
    pgraph_vk_transition_image_layout(pg, cmd,
                                      disp->swapchain_images[image_index],
                                      disp->swapchain_format, old_swap_layout,
                                      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    ...
    vkCmdClearColorImage(cmd, disp->swapchain_images[image_index],
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clear, 1,
                         &range);

    pgraph_vk_transition_image_layout(pg, cmd,
                                      disp->swapchain_images[image_index],
                                      disp->swapchain_format,
                                      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                      VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
    disp->swapchain_image_layouts[image_index] =
        VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;

    if (!pgraph_vk_android_end_present_commands(pg, cmd)) {
        return;
    }
    ...
    VkPresentInfoKHR present_info = {
        ...
        .pWaitSemaphores = &disp->render_finished,
        .swapchainCount = 1,
        .pSwapchains = &disp->swapchain,
        .pImageIndices = &image_index,
    };
    result = android_vkQueuePresentKHR(r->queue, &present_info);
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        pgraph_vk_android_create_swapchain(pg);
        return;
    }
    if (result != VK_SUCCESS) {
        xemu_android_log_warn("Android Vulkan placeholder: present failed: %d", result);
        return;
    }

    disp->present_count++;
    xemu_android_log_info("Android Vulkan placeholder presented frame=%d target=%ux%u",
                          disp->present_count, disp->swapchain_extent.width,
                          disp->swapchain_extent.height);
}

static void pgraph_vk_android_present(PGRAPHState *pg)
{
    PGRAPHVkState *r = pg->vk_renderer_state;
    PGRAPHVkDisplayState *disp = &r->display;

    if (disp->image == VK_NULL_HANDLE) {
        return;
    }
    ...
    if (result != VK_SUCCESS && result != VK_SUBOPTIMAL_KHR) {
        xemu_android_log_warn("Android Vulkan present: acquire failed: %d", result);
        return;
    }
    ...
    uint32_t target_width = disp->swapchain_extent.width;
    uint32_t target_height = disp->swapchain_extent.height;
    uint32_t dst_width = target_width;
    uint32_t dst_height = target_height;
    uint32_t dst_x = 0;
    uint32_t dst_y = 0;

    if (disp->width > 0 && disp->height > 0 &&
        target_width > 0 && target_height > 0) {
        uint64_t target_times_source_h =
            (uint64_t)target_width * disp->height;
        uint64_t target_h_times_source_w =
            (uint64_t)target_height * disp->width;

        if (target_times_source_h > target_h_times_source_w) {
            dst_width = (uint32_t)(((uint64_t)target_height * disp->width) /
                                   disp->height);
            dst_x = (target_width - dst_width) / 2;
        } else if (target_times_source_h < target_h_times_source_w) {
            dst_height = (uint32_t)(((uint64_t)target_width * disp->height) /
                                    disp->width);
            dst_y = (target_height - dst_height) / 2;
        }
    }
    ...
    if (result != VK_SUCCESS) {
        xemu_android_log_warn("Android Vulkan present: present failed: %d", result);
        return;
    }
    ...
    if ((disp->present_count % 60) == 1) {
        xemu_android_log_info("Android Vulkan presented frame=%d source=%dx%d target=%ux%u viewport=%ux%u+%u+%u",
                              ...
                              disp->swapchain_extent.width,
                              disp->swapchain_extent.height, dst_width,
                              dst_height, dst_x, dst_y);
    }
}
#endif
...
void pgraph_vk_render_display(PGRAPHState *pg)
{
    ...
    PGRAPHVkState *r = pg->vk_renderer_state;
#ifdef __ANDROID__
    static int android_render_attempts;
    android_render_attempts++;
#endif
    ...
    if (surface == NULL || !surface->color || !surface->width ||
        !surface->height) {
#ifdef __ANDROID__
        if ((android_render_attempts % 30) == 1) {
            xemu_android_log_warn(
                "Android Vulkan render skipped frame=%d surface=%p color=%d size=%dx%d pcrtc=0x%08x line=%d",
                android_render_attempts, surface,
                surface ? surface->color : 0,
                surface ? surface->width : 0,
                surface ? surface->height : 0,
                d->pcrtc.start, vga_display_params.line_offset);
        }
        pgraph_vk_android_present_placeholder(pg);
#endif
        return;
    }
    ...
    if ((android_render_attempts % 30) == 1) {
        xemu_android_log_info(
            "Android Vulkan render frame=%d source=%dx%d output=%ux%u pcrtc=0x%08x line=%d",
            android_render_attempts, surface->width, surface->height, width,
            height, d->pcrtc.start, vga_display_params.line_offset);
    }
#endif
